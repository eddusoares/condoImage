...
class FrontendController extends Controller
{
    ...
    public function frontendSections($key)
    {
        ...
        $optionalContentFieldsMap = [
            'category' => ['subheading', 'show_meta', 'limit'],
            'neighborhood_gallery' => ['subheading', 'show_meta', 'limit'],
        ];
        $optionalContentFields = $optionalContentFieldsMap[$key] ?? [];
        return view('admin.frontend.index', compact('section', 'content', 'elements', 'key', 'pageTitle', 'optionalContentFields'));
    }
    ...
    public function frontendContent(Request $request, $key)
    {
        ...
        foreach ($request->except('_token', 'video') as $input_field => $val) {
            ...
            if (isset($optionalFields[$key]) && in_array($input_field, $optionalFields[$key])) {
                continue;
            }
