...
class GeneralSettingController extends Controller
{
    ...
    public function watermarkUpdate(Request $request)
    {
        ...
        $general = GeneralSetting::first();
        $general->watermark_status = $request->input('watermark_status') ? 1 : 0;
        $general->save();
    ...
    {
        $request->validate([
            'image_resolution' => ['nullable', 'regex:/^\d{2,4}x\d{2,4}$/'],
            'image_compression' => ['nullable', 'min:0','max:100']
        ]);
        $general = GeneralSetting::first();
        $general->image_resolution = $request->image_resolution;
        $general->image_compression = $request->image_compression;
        $general->save();

        $notify[] = ['success', 'Image resolution has been changed successfully'];
        return redirect()->to(url()->previous() . '#refresh')->withNotify($notify);
    }
