...
class ReviewController extends Controller
{
    public function pending()
    {
        ...
        return view('reviewer.review.index', compact('pageTitle', 'files'));
    }
    ...
    public function detail($id)
    {
        ...
        return view('reviewer.review.detail', compact('pageTitle', 'file', 'categories'));
    }
    ...
    {
        $pageTitle = 'On Reviewing Files';
        $files = File::where('status', 3)->where('reviewer_id', auth()->guard('reviewer')->user()->id)->with(['user', 'category'])->orderBy('updated_at', 'desc')->paginate(getPaginate());
        return view('reviewer.review.index', compact('pageTitle', 'files'));
    }

    public function published()
    {
        $pageTitle = 'Published Files';
        $files = File::where('status', 1)->where('reviewer_id', auth()->guard('reviewer')->user()->id)->with(['user', 'category'])->orderBy('updated_at', 'desc')->paginate(getPaginate());
        return view('reviewer.review.index', compact('pageTitle', 'files'));
    }

    public function rejected()
    {
        $pageTitle = 'Rejected Files';
        $files = File::where('status', 2)->where('reviewer_id', auth()->guard('reviewer')->user()->id)->with(['user', 'category'])->orderBy('updated_at', 'desc')->paginate(getPaginate());
        return view('reviewer.review.index', compact('pageTitle', 'files'));
    }
    ...
    public function changeHandleStatus(Request $request)
    {
        ...
        if (isset($request->progress)) {
            $file->reviewer_id = auth()->guard('reviewer')->user()->id;
            $file->status = 3;
        } else {
