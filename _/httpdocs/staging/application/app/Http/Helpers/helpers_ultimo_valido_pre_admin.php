...
function fileChunkDelete($filename, $uuid)
{
    ...
    $size = gs()->image_resolution ?? '800x600';
    $globalImageCompression = gs()->image_compression;
    list($width, $height) = explode('x', strtolower($size));
    $image = Image::make($finalPath)->resize($width, $height)->save($finalPath, $globalImageCompression);
...
function awsUploader($tempPath, $fileName, $imageWatermarkSize)
{
    ...
    if (gs()->watermark_status) {
        ...
        Image::make($tempPath)
            ...
            ->save($tempWatermarkedImagePath,$globalImageCompression);  //store tempory file
...
function localUploader($sourcePath, $newPath, $fileName, $imageWatermarkSize, $watermarkDir)
{
    ...
    if (gs()->watermark_status) {
        ...
        $globalImageCompression = gs()->image_compression;
        list($width, $height) = explode('x', strtolower($size));
        $mainImage = Image::make($newPath)->resize($width, $height)->save($newPath, $globalImageCompression);
