...
class FileManager
{
    ...
    */
    ...
    protected function uploadImage()
    {
        ...
        if ($this->watermark) {
            if ($this->old) {
                $this->removeFile($this->path . '/watermark/watermark_' . $this->old);
            }
            ...
            $this->makeDirectory($this->path . '/watermark');
            $mainImage->insert($watermarkImage, 'center')->save($this->path . '/watermark/watermark_' . $this->filename);
        }
    }
    ...
    public function removeFile($path = null)
    {
        ...
        if ($this->watermark) {
            if (!$path) $path = $this->path . '/watermark/watermark_' . $this->old;
            file_exists($path) && is_file($path) ? @unlink($path) : false;
