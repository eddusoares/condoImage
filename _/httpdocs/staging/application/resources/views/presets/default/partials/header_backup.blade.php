    ...
    <div class="container">
        <nav class="navbar">
            <div class="header-menu-wrapper">
                <a href="{{ route('home') }}" class="brand-text" id="normal-logo">
            ...
            <div class="nav-actions ms-auto d-flex align-items-center gap-2 gap-md-3">
                <div class="navbar-search">
                    <button class="navbar-search__toggle" id="navbarSearchToggle" type="button" aria-haspopup="true"
                        aria-expanded="false" aria-label="Open search" data-open-sidebar="search">
                        <i class="fas fa-search"></i>
                    </button>

                    <div class="navbar-search__panel" id="navbarSearchPanel" aria-hidden="true" data-sidebar="search" 
                         role="dialog" aria-modal="true">
                        <div class="navbar-search__card">
                            <form action="{{ route('search.building') }}" method="GET" class="navbar-search__field">
                                <i class="fas fa-search"></i>
                                <input id="navbarSearchInput" name="search" type="text" data-suggest="true" data-building-search
                                    placeholder="Search buildings and neighborhoods" autocomplete="off"
                                    aria-autocomplete="list" aria-expanded="false" aria-owns="building-suggestions"
                                    data-suggest-data="#buildings-data"
                                    data-suggest-list="#building-suggestions"
                                    data-suggest-template="#headerSuggestionTemplate"
                                    data-suggest-max="4"
                                >
                                <button class="navbar-search__close" id="navbarSearchClose" type="button"
                                    aria-label="Close search" data-close-sidebar>
                                    <i class="las la-times"></i>
                                </button>
                            </form>
                            ...
                            <template id="headerSuggestionTemplate">
                                <li role="option"><a href="#" data-role="item-link"><i class="las la-arrow-right"></i> Explore <span data-role="item-text"></span></a></li>
                            </template>
...
{!! json_encode($searchItems->values(), JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES) !!}
                ...
                <a href="{{ route('user.login') }}" class="nav-icon nav-actions__account" aria-label="Account"><i
                        class="far fa-user"></i></a>
                <button class="navbar-toggler header-button nav-actions__toggler" type="button"
                    id="mobileMenuToggle" aria-label="Toggle navigation" data-open-sidebar="menu">
...
<div class="mobile-menu-system" id="mobileMenuSystem" data-sidebar="menu" 
     role="dialog" aria-modal="true" aria-hidden="true">
    <div class="mobile-menu-overlay" data-mobile-overlay></div>
    <!-- Mobile Search Menu -->
    <div class="mobile-menu-panel" id="mobileSearchPanel">
        <div class="mobile-menu-header">
            <div></div>
            <button class="mobile-menu-close" data-action="close" data-close-sidebar>
                <i class="las la-times"></i>
            </button>
        </div>
                    ...
                    @foreach($allBuildings->take(4) as $building)
                        <li>
                            <a href="{{ route('condo.building.details', building_route_params($building)) }}">
                                <i class="las la-arrow-right"></i> Explore {{ $building->name }}
                            </a>
                        </li>
                    @endforeach
                </ul>
        ...
        <div class="mobile-menu-content">
            ...
            <div class="mobile-menu-section">
                ...
                <ul class="mobile-menu-list">
                    @foreach($splitNeighborhoods->flatten()->take(5) as $neighborhood)
                        <li>
                            <a href="{{ route('neighborhood.details', ['county' => slug($neighborhood->county->name), 'slug' => slug($neighborhood->name), 'id' => $neighborhood->id]) }}" class="mobile-menu-link">
                                {{ $neighborhood->name }}
                            </a>
                        </li>
                    @endforeach
            ...
            <div class="mobile-menu-section">
                ...
                <ul class="mobile-menu-list mobile-menu-list--actions">
                    <li>
                        <a href="{{ route('condo.building') }}" class="mobile-menu-action">
                            View all condos
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('neighborhood') }}" class="mobile-menu-action">
    ...
    <div class="mobile-menu-panel" id="mobileCondoPanel">
        ...
        <div class="mobile-menu-content">
            ...
            <div class="mobile-menu-section">
                ...
                <ul class="mobile-menu-list">
                    @foreach($splitBuildings->flatten()->take(7) as $building)
                        <li>
                            <a href="{{ route('condo.building.details', building_route_params($building)) }}" class="mobile-menu-link">
                                {{ $building->name }}
                            </a>
                        </li>
                    @endforeach
...
@push('script')
    <script>
        ...
        <script src="{{ asset($activeTemplateTrue . 'js/suggestions.js') }}"></script>
@endpush

<!-- Mobile Menu Script removed - replaced by sidebar-system.js -->
...
<style>
    ...
    .mobile-menu-system.active .mobile-menu-overlay {
        opacity: 1;
        pointer-events: auto;
    }
    ...
    .mobile-menu-panel {
        ...
        box-sizing: border-box; /* Include padding within width for precise positioning */
        height: 100vh;
        height: 100dvh; /* Dynamic viewport height for mobile */
        background: #F7F7F8;
        transition: transform 0.35s cubic-bezier(0.25, 0.46, 0.45, 0.94); /* Smooth curtain animation */
        display: flex;
        flex-direction: column;
        opacity: 0;
        visibility: hidden;
    ...
    .mobile-menu-panel.active {
        opacity: 1;
        visibility: visible;
        transform: translateX(0); /* Slide into viewport, stopping exactly at edge */
    }
    ...
    body.mobile-menu-open {
        ...
        height: 100vh;
        height: 100dvh;
        position: fixed; /* Prevent background scroll and viewport shift */
        width: 100%;
        max-width: 100vw;
        left: 0;
        right: 0;
