                ...
                <div id="buildingCarousel" class="tc-carousel">
                    <div class="tc-carousel-track">
                        @if($buildingImages->count())
                            @foreach ($buildingImages as $index => $img)
                                <div class="tc-card" data-modal-trigger data-image-index="{{ $index }}">
...
<div id="imageModal" class="image-modal" style="display: none;" aria-hidden="true" role="dialog" aria-labelledby="modalTitle" aria-describedby="modalDescription">
    ...
    <div class="image-modal__container">
        <div class="image-modal__header">
            <h3 id="modalTitle" class="image-modal__title">{{ $building->name }} - Images</h3>
...
function normalizeImageData() {
    ...
    @if($buildingImages->count())
        @foreach ($buildingImages as $img)
            images.push({
                src: "{{ getImage(getFilePath('building') . '/' . $img) }}",
                alt: "{{ addslashes($building->name) }}"
            });
        @endforeach
    @else
        images.push({
            src: "{{ getImage(getFilePath('building') . '/' . $building->image) }}",
            alt: "{{ addslashes($building->name) }}"
        });
    @endif
