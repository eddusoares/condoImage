@php
    $tc = getContent('top_categories_neighborhood.content', true);
    
    // Use neighborhoods from database with admin parameters
    try {
...
<section class="top-categories-container pb-100" id="top-categories-neighborhood">
    <div class="container-fluid">
        <div class="row align-items-center g-5">
            <div class="col-lg-6">
                <div class="tc-copy">
                    <h2 class="tc-title">{{ __($tc->data_values->heading ?? 'Discover neighborhoods') }}</h2>
                    <p class="tc-subtitle">{{ __($tc->data_values->subheading_primary ?? 'Explore premium neighborhoods and find your perfect location. From downtown areas to suburban communities.') }}</p>
                    <p class="tc-subtitle-secondary mb-4">{{ __($tc->data_values->subheading_secondary ?? 'Find the best neighborhoods for your lifestyle.') }}</p>
                    <a href="{{ $tc->data_values->button_link ?? url('/neighborhood') }}" class="tc-button">{{ __($tc->data_values->button_text ?? 'Explore all neighborhoods') }}</a>
                </div>
...
<script>
    (function () {
        ...
        function initSection(section) {
            ...
            // Regra: com N slides existem N-1 cliques possíveis para avançar/voltar
            let clickPos  = 0;                         // posição de clique acumulada
            ...
            function recalc() {
                ...
                // Regra fixa por total de imagens: N → N-1 cliques
