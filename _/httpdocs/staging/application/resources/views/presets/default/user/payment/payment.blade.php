@php
    ...
    $payableMoney = $payment == 1 ? $general->image_price : $general->package_price;
@endphp
            ...
            <div class="row d-flex justify-content-center">
                <div class="col-lg-6">
                    <div class="dashboard-card-wrap mt-0">
                        <form id="submitForm" action="{{ route('user.deposit.insert') }}" method="post">
                            @csrf
                            ...
                            <div class="support-ticket-border">
                                <h5 class="card-title">@lang('Payment')</h5>
                                <div class="card-body">
                                    <div class="form-group mb-3">
                                        <label class="form--label">@lang('Select Gateway')</label>
                                        <select id="gateway" class="form--control select" name="gateway" required>
                                            <option value="">@lang('Select One')</option>
                                            <option value="0">@lang('Pay with Balance') -
                                                {{ $general->cur_sym }}{{ showAmount($balance) }}</option>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <input type="number" step="any" name="amount"
                                                class="form-control form--control" value="{{ $payableMoney }}"
                                                autocomplete="off" readonly required>
                                            <span
                                                class="input-group-text bg--base border-0 text-light">{{ $general->cur_text }}</span>
                                        </div>
                                    </div>
                                    <div class="mt-3 preview-details d-none">
                                        <ul class="list-group">
                                            <li class="d-flex justify-content-between modal-background mb-3">
                                            ...
                                            <li class="justify-content-between d-none rate-element modal-background mb-3">

                                            </li>
                                            <li class="justify-content-between d-none in-site-cur modal-background mb-3">
                                                <span>@lang('In') <span class="base-currency"></span></span>
                                                <span class="final_amo fw-bold">0</span>
                                            </li>
                                            <li class="justify-content-center crypto_currency d-none modal-background mb-3">
                                    ...
                                    <button type="submit" class="btn button w-100 mt-3">@lang('Submit')</button>
                                </div>
                            </div>
                        </form>
                    </div>
    ...
    <script>
        (function($) {
            ...
            $('select[name=gateway]').change(function() {
                ...
                if (selectedValue != 0) {
                    $('#submitForm').attr('action', "{{ route('user.deposit.insert') }}");
                    ...
                    $('.preview-details').addClass('d-none');
                    $('#submitForm').attr('action', "{{ route('user.pay.balance') }}");
                }
            });
