...
class ListingAssetController extends Controller
{
    ...
    {
        $pageTitle = 'My  Listing Units';
        $buildings = Building::where('status', 1)->orderBy('id', 'desc')->get();
        $myListingUnits = ListingUnit::where('userable_type', 'user')
            ->where('userable_id', auth()->id())->orderBy('id', 'desc')->paginate(getPaginate(20));
        return view($this->activeTemplate . 'user.listing_image.my_list', compact('pageTitle', 'myListingUnits', 'buildings'));
    }
    ...
    public function create()
    {
        ...
        $buildings = Building::where('status', 1)->orderBy('id', 'desc')->get();
        $listingUnits = ListingUnit::with('building')->orderBy('id', 'desc')->paginate(getPaginate(20));
        return view($this->activeTemplate . 'user.listing_image.create', compact('pageTitle', 'listingUnits', 'buildings'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $listingUnit->userable_id = auth()->id();
            $listingUnit->userable_type = 'user';
            $listingUnit->price = $request->price;
            $listingUnit->zip_url = asset($zipUrl);
            $listingUnit->is_premium = $request->is_premium ? 1 : 0;
            $listingUnit->step = 1;
            ...
            return to_route('user.listing.asset.upload', $listingUnit->id)->withNotify($notify);
        } catch (\Exception $exp) {
            DB::rollBack();
    ...
    public function imageUpload($id)
    {
        ...
        abort_unless($listingUnit->userable_type === 'user' && $listingUnit->userable_id === auth()->id(), 403, 'Unauthorized action.');
        return view($this->activeTemplate . 'user.listing_image.image_upload', compact('pageTitle', 'listingUnit'));
    }
    ...
    public function uploadChunk(Request $request)
    {
        ...
        if (!$request->has('dztotalchunkcount')) {
            ...
            return response()->json([
                ...
                'uuid' => $uuid,
                'name' => $filename,
                'finalize_url' => route('user.listing.asset.upload.store')
            ]);
        }
    ...
    public function upload(Request $request)
    {
        ...
        if (File::exists($sourcePath)) {
            ...
            $listingImage->userable_id = auth()->id();
            $listingImage->userable_type = 'user';
    ...
    public function edit($id)
    {
        ...
        $listingUnit = ListingUnit::with('listingImages')->where('id', $id)->orderBy('id', 'desc')->first();
        abort_unless($listingUnit->userable_type === 'user' && $listingUnit->userable_id === auth()->id(), 403, 'Unauthorized action.');
        return view($this->activeTemplate . 'user.listing_image.edit', compact('pageTitle', 'listingUnit', 'buildings'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        abort_unless($listingUnit->userable_type === 'user' && $listingUnit->userable_id === auth()->id(), 403, 'Unauthorized action.');
        $hasExistingImages = $listingUnit->listingImages()->exists();
        $purifier = new \HTMLPurifier();
        ...
        $this->validate($request, [
            ...
            'images.required' => 'At least one listing image is required since no image exists.',
            'images.min' => 'The images field must have at least 1 item.',
        ]);
    ...
    public function ListingImageDelete($id)
    {
        ...
        if (!($listingImage->userable_id == auth()->id() && $listingImage->userable_type == 'user')) {
